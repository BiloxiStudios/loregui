...
- **Four-action first-run hub** — **Open existing**, **Create local**, **Connect**, or **Host** — with repository validation, no-auth LAN support, and native Browse. The **Host** flow has a guided config surface: a one-field Basic mode for a local server, and an Expert mode exposing all 54 lore-server options (network/QUIC/gRPC/HTTP, storage, topology & mTLS replication, telemetry, runtime, notifications, features, timeouts). LoreGUI generates the `loreserver` TOML for you — view it with one click; you never hand-edit config.
