...
The **server store** selected in this flow belongs to `loreserver`; it is not
the local working tree used by Branches, Sync, Commit, or other repository
actions. After hosting succeeds, choose one explicit next action: **Open
existing**, **Create local**, **Connect**, or **Manage server only**. Create/Open
uses a separate client local path selected with the native **Browse** picker.
...
The running-server card keeps the effective name, advertised URL, store path,
authentication mode, and process status visible. **Authentication: Not
required** is a valid standalone/no-auth state, not an error. **Browse
repositories** uses that card's advertised URL directly rather than asking you
to type it again.
